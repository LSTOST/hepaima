...
export default function AdminDashboardPage() {
  ...
  const fetchOverviewStats = async (pwd: string) => {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.message ?? "获取统计失败");
  ...
  useEffect(() => {
    if (authenticated && password && tab === "overview") {
      fetchOverviewStats(password);
    }
  }, [authenticated, password, tab]);
  ...
  const handleDeleteProduct = async (productId: string) => {
    ...
    if (
      !p ||
      !window.confirm(
        `确定删除产品「${p.name}」？将同时删除其下所有问卷、题目、维度与 AI 模板，此操作不可恢复。`
      )
    )
      return;
      ...
      <div className="max-w-[1100px] mx-auto px-4 sm:px-6 py-6">
        ...
        {tab === "analytics" && (
          <div className="space-y-5">
                ...
                <div className="space-y-4 text-sm">
                  <div className="flex flex-wrap items-center gap-4">
                    <label className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        checked={siteSettings.enableGa4 ?? false}
                        onChange={(e) =>
                          setSiteSettings({ ...siteSettings, enableGa4: e.target.checked })
                        }
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-500 mb-1">GA4 测量 ID（如 G-XXXXXXXXXX）</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-500 mb-1">Umami Script URL（如 https://umami.example.com/script.js）</label>
                    <input
                      type="text"
                      value={siteSettings.umamiScriptUrl ?? ""}
                      onChange={(e) =>
                        setSiteSettings({ ...siteSettings, umamiScriptUrl: e.target.value || undefined })
                      }
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-500 mb-1">Umami Website ID（data-website-id）</label>
                    <input
                      type="text"
                      value={siteSettings.umamiWebsiteId ?? ""}
                      onChange={(e) =>
                        setSiteSettings({ ...siteSettings, umamiWebsiteId: e.target.value || undefined })
                      }
            ...
            <div className="rounded-xl bg-white border border-slate-200 p-5">
              <h3 className="text-sm font-semibold text-slate-700 mb-2">自建埋点事件类型</h3>
              <p className="text-xs text-slate-500 mb-2">
                前端可调用 <code className="bg-slate-100 px-1 rounded">POST /api/v1/track</code> 上报事件，用于总览中的流量与事件统计。
              </p>
              <ul className="text-xs text-slate-600 space-y-1">
                <li><code className="bg-slate-100 px-1 rounded">page_view</code> 页面浏览</li>
                <li><code className="bg-slate-100 px-1 rounded">quiz_start</code> 开始测评</li>
                <li><code className="bg-slate-100 px-1 rounded">quiz_complete</code> 完成测评</li>
                <li><code className="bg-slate-100 px-1 rounded">result_view</code> 查看报告</li>
                <li><code className="bg-slate-100 px-1 rounded">pay_click</code> 点击付费</li>
                <li><code className="bg-slate-100 px-1 rounded">redeem_use</code> 使用兑换码</li>
              </ul>
              <p className="text-xs text-slate-400 mt-2">
                请求体示例：<code className="block mt-1 bg-slate-100 p-2 rounded text-[11px]">{`{ "eventType": "page_view", "path": "/quiz", "visitorId": "可选访客ID" }`}</code>
              </p>
            </div>
          </div>
        )}
                    ...
                    <div className="flex items-center justify-between gap-3 flex-wrap">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          disabled={deletingProductId === currentProduct.id}
                        >
                          {deletingProductId === currentProduct.id ? "删除中..." : "删除产品"}
                        </Button>
                      </div>
                    </div>
